...
pub fn compute_features(
    ...
    features::compute_features(candles_5m, candles_1h)
}
...
pub async fn run_once_core(
            ...
            .await
        {
                ...
                if e.downcast_ref::<account::BinanceApiError>().is_some() || is_auth_error(&e) =>
            {
                ...
                if let Some(be) = e.downcast_ref::<account::BinanceApiError>() {
                    ...
                    debug!(
                        "Wallet access failed: http_status={} binance_code={:?} binance_msg={:?}",
                        be.status, be.code, be.msg
                    );
                    if let (Some(-2015), _) = (be.code, be.msg.as_deref()) {
                ...
                return Ok(CoreOutcome {
                    ...
                    reason: "I cannot see your wallet. Binance rejected the key.".to_string(),
    ...
    let f = match compute_features(&md.candles_5m, &md.candles_1h) {
        Ok(x) => x,
        Err(e) => {
    ...
    let equity_usdt = bals.usdt_free + (bals.btc_free * md.price);
    ...
    match st.position.clone() {
        state::Position::Flat => {
            ...
            if st.trades_today >= max_trades_per_day {
                ...
                return Ok(CoreOutcome {
                    ...
                    regime: r.regime,
                    decision: "WAIT".to_string(),
                    reason: "We wait. We already traded enough today.".to_string(),
                    end: "no money moved".to_string(),
                });
            }

            let sig = signals::entry_long_signal(r.regime, &f, md.price, r.vol_ratio);
                ...
                match sized {
                    ...
                    risk::RiskDecision::Allow {
                        ...
                        if cap > 0.0 && notional > cap {
                            let capped_qty =
                                sizing::round_down_to_step(cap / md.price, md.step_size);
                            if capped_qty > 0.0 {
                                qty = capped_qty;
                                notional = md.price * qty;
                                was_capped = true;
                            }
                        }
                        ...
                        st.enter_long(entry_price, entry_qty, stop, md.now_ms);
        ...
        state::Position::Long {
            ...
            let (decision, why) = if range_tp {
                ...
                st.manage_open_position(md.price, f.atr14_5m, md.now_ms)
            };
            if decision == state::PositionDecision::ExitLong {
                ...
                let sell_qty = sizing::round_down_to_step(qty.min(bals.btc_free), md.step_size);
                if sell_qty <= 0.0 {
    ...
    if have_keys && mode == Mode::Live && did_place_order {
        ...
        {
            Ok(bals2) => {
                let equity2 = bals2.usdt_free + (bals2.btc_free * md.price);
